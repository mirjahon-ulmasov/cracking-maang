...
// Same approach using reduce function
function timeRequiredToBuy3(tickets: number[], k: number): number {
    let ticket = tickets[k]

    return tickets.reduce((acc, cur, index) => {
        if (index <= k) acc += Math.min(ticket, cur)
        else acc += Math.min(ticket - 1, cur)
        return acc
    }, 0)
}
